...
setwd("E:/")
load("vfm/mortality_20170320_5.Rdata")
...
# (3) simplified model # 8 predictors (>90% r^2?) + female
vars <- c("rcs(age, 4) + meds + rcs(haemoglobin, 4) + cancer + rcs(log.urine_acr, 4) + smoking + rcs(duration, 4) + stroke + female")
fm <- as.formula(paste("Surv(years, event) ~", vars))
...
mortality <- model
save(mortality, file = "vfm/mortality_model.Rdata")
