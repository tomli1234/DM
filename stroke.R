...
rm(list=setdiff(ls(), c("d", "imp", "fm1", "folder")))
fm <- update(fm1, ~. -rcs(log.egfr_chinese, 4) -rcs(sbp, 4) -rcs(map, 4) -rcs(lr, 4) -rcs(hdl, 4) -rcs(ldl, 4))
...
fm <- as.formula(Surv(years, event) ~ rcs(age, 4)*rcs(log.urine_acr, 4) + rcs(age, 4)*smoking)
