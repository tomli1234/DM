...
stroke <- fit.mult.impute(fm, fitter = cph, xtrans = imp, data = d, x=TRUE, y=TRUE, surv=TRUE, time.inc=5)

#estimation curve for variable
ggplot(Predict(stroke), sepdiscrete ='vertical', nlevels=4, vnames ='names')
...
completed$pred <- 1 - predictSurvProb(stroke, completed, times=5)
