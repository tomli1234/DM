...
c.stat <- Cindex_full(model = fullmodel, bootstrap = 3) # Should be 100
```

**Model Calibration**
  
```{r calibration, warning = FALSE}
Calibrate_full(model = fullmodel, bootstrap = 3) # Should be 100
...
boots_results <- valid_approx(3, fm = fm, completedata = completed) # Should be 100

      # Discrimination
c_index <- boots_results$validation
c_index_CI <- quantile(c_index, c(0.025, 0.5, 0.975))

      # Calibration
p1 <- plot_cal_bar(boots_results$calibration, colours)
p1
```
...
save(list=c("boots_results", "c.stat", "approx", "fullmodel"), file = paste0( folder, filename, ".RData"))
