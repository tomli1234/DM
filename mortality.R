...
#22 predictors for mortality, 20 variables+stroke+chd, no interaction terms are conisdered 
nk <- 4
vars <- c("rcs(age, nk) + rcs(duration, nk) + rcs(log.urine_acr, nk) + rcs(bmi, nk) + rcs(haemoglobin, nk) + rcs(wbc, nk) + rcs(pulse, nk) + rcs(lr, nk) + rcs(hba1c, nk) + rcs(log.creatinine, nk) + rcs(map, nk) + rcs(triglyceride, nk) +smoking + af + cancer + pad + ckd + meds + complications + female + stroke + chd")
vars <- gsub("nk", nk, vars)
fm <- as.formula(paste("Surv(years, event) ~", vars))
mortality <- fit.mult.impute(fm, fitter = cph, xtrans = imp, data = d, x=TRUE, y=TRUE, surv=TRUE, time.inc=5)

#estimation curve for variable
ggplot(Predict(mortality), sepdiscrete ='vertical', nlevels=4, vnames ='names')
#importance of each variable
plot(anova(mortality))
#estimation result
mortality

#5-year risk prediction
#if we want to predict 10-year risk, change times=5 to times=10.
completed$pred <- 1 - predictSurvProb(mortality, completed, times=5)
